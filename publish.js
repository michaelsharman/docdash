...
function needsSignature(doclet) {
    ...
    // bit messy)
    else if (doclet.kind === 'namespace' && doclet.meta && doclet.meta.code && doclet.meta.code.type && doclet.meta.code.type.match(/[Ff]unction/)) {
        needsSig = true;
    }
...
function addAttribs(f) {
    ...
    if (attribsString && attribsString.length) {
        f.attribs = util.format(
            '<span class="type-signature type-signature-' + attribsString.replace(/\(/g, '').replace(/\)/g, '').trim() + '">%s</span>',
            attribsString
        );
    } else {
...
function buildMemberNav(items, itemHeading, itemsSeen, linktoFn) {
    ...
    if (items && items.length) {
        ...
        let renderedAssessmentModules = false;
        let renderedAuthoringModules = false;
        let renderedUtilsModules = false;
        let renderedExtensionsAssessmentHeading = false;
        let renderedExtensionsAuthoringHeading = false;
        ...
        items.forEach(function (item) {
            ...
            if (!hasOwnProp.call(item, 'longname')) {
                ...
                if (displayName.includes('Assessment') && !displayName.includes('_Extensions') && !renderedAssessmentModules) {
                    renderedAssessmentModules = true;
                    itemsNav += '<h3>Items API</h3></li>';
                    itemsNav += '<li' + classes + '>';
                }
                if (displayName.includes('Authoring') && !displayName.includes('_Extensions') && !renderedAuthoringModules) {
                    renderedAuthoringModules = true;
                    itemsNav += '<h3>Author API</h3></li>';
                    itemsNav += '<li' + classes + '>';
                }
                if (displayName.includes('Utils') && !renderedUtilsModules) {
                    renderedUtilsModules = true;
                    itemsNav += '<h3>Utils</h3></li>';
                    itemsNav += '<li' + classes + '>';
                }

                if (displayName.includes('_Extensions/Assessment') && !renderedExtensionsAssessmentHeading) {
                    renderedExtensionsAssessmentHeading = true;
                    itemsNav += '<h3>Extensions - Items API</h3></li>';
                    itemsNav += '<li' + classes + '>';
                }
                if (displayName.includes('_Extensions/Authoring') && !renderedExtensionsAuthoringHeading) {
                    renderedExtensionsAuthoringHeading = true;
                    itemsNav += '<h3>Extensions - Author API</h3></li>';
                    itemsNav += '<li' + classes + '>';
                }
                displayName = displayName.replace('_Extensions/Assessment/', '');
                displayName = displayName.replace('_Extensions/Authoring/', '');
                displayName = displayName.replace('Assessment/', '');
                displayName = displayName.replace('Authoring/', '');
                displayName = displayName.replace('Utils/', '');
...
function buildNav(members) {
    ...
    var defaultOrder = ['Classes', 'Modules', 'Externals', 'Events', 'Namespaces', 'Mixins', 'Tutorials', 'Interfaces', 'Global'];
...
exports.publish = function (taffyData, opts, tutorials) {
    ...
    generate(
        '',
        'Home',
        packages.concat([{ kind: 'mainpage', readme: opts.readme, longname: opts.mainpagetitle ? opts.mainpagetitle : 'Main Page' }]).concat(files),
        indexUrl
    );
